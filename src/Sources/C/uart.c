...
//轮询法发送一帧数据
//消耗时间与数据长度有关
//不可以放在中断里面周期性调用，因为发送时间过长，扰乱中断正常进行
//消耗时间计算看帮助文档
//---------------------------------------------
void sendDataToScope(void)
{
  ...
  uart_send1(FreeCarsUARTPort,251);
  uart_send1(FreeCarsUARTPort,109);
  uart_send1(FreeCarsUARTPort,37);
  sum+=(251);      //全部数据加入校验
  sum+=(109);
  sum+=(37);
  for(i=0;i<FreeCarsDataNum;i++)
  {
    uart_send1(FreeCarsUARTPort,uSendBuf[i]);
    sum+=uSendBuf[i];         //全部数据加入校验
  }
  uart_send1(FreeCarsUARTPort,sum);
}
